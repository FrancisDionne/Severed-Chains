...
public class TooManyItemsScreen extends MenuScreen {
  ...
  protected void render() {
    switch(this.menuState) {
      ...
      case RENDER_4 -> {
        menuStack.pushScreen(new MessageBoxScreen(I18n.translate(TOO_MANY_ITEMS_CONFIRM), 2, result -> this.setMenuState(result.messageBoxResult == MessageBoxResult.YES ? MenuState.RENDER_6 : MenuState.DISCARD_10)));
      ...
      case INVENTORY_9 -> {
        ...
        if(this.droppedItems.get(this.dropIndex).item_00 instanceof Equipment) {
          slotCount = getUniqueInventoryEquipments().size();
        } else {
          slotCount = getUniqueInventoryItems().size();
        }
  ...
  private void renderItemLists(final boolean allocate, final InventoryEntry<?> inv, final int mode) {
    ...
    renderMenuItems(16, 33, this.droppedItems, this.dropScroll, Math.min(DROPPED_ITEM_LIST_SIZE, this.droppedItems.size()), this.dropUpArrow, this.dropDownArrow, false, false);
  ...
  private void renderCurrentItemList(final boolean allocate, final String text, final MenuEntries<? extends InventoryEntry<?>> entries, final int mode) {
    ...
    if((mode & 0x1) != 0) {
      renderMenuItems(194, 33, entries, this.invScroll, INVENTORY_ITEM_LIST_SIZE, this.invUpArrow, this.invDownArrow, false, false);
    }
  ...
  private void selectMenuState9() {
    ...
    if(((isItem ? this.items : this.equipment).get(this.invIndex + this.invScroll).flags_02 & 0x6000) != 0) {
      ...
      if(getInventoryEntryQuantity(isItem ? (ItemStack)discardedItem : (Equipment)discardedItem) > 0) {
        if(!compareInventoryEntries(isItem ? (ItemStack)discardedItem : (Equipment)discardedItem, isItem ? (ItemStack)newItem.item_00 : (Equipment)newItem.item_00)) {
          if(isItem) {
            this.items.add((MenuEntryStruct04<ItemStack>)(MenuEntryStruct04)newItem);
          } else {
            this.equipment.add((MenuEntryStruct04<Equipment>)(MenuEntryStruct04)newItem);
          }
        }
      } else {
        if(getInventoryEntryQuantity(isItem ? (ItemStack)newItem.item_00 : (Equipment)newItem.item_00) < 1) {
          if(isItem) {
          ...
          (isItem ? this.items : this.equipment).remove(this.invIndex + this.invScroll);
        }
      }

      if(isItem) {
        gameState_800babc8.items_2e9.give((ItemStack)newItem.item_00);
      } else {
        gameState_800babc8.equipment_1e8.add((Equipment)newItem.item_00);
      }
  ...
  private void sortMenuState9() {
    ...
    if(this.droppedItems.get(this.dropScroll + this.dropIndex).item_00 instanceof Equipment) {
      final List<Equipment> list = getUniqueInventoryEquipments();
      sortItems(this.equipment, list);
    } else {
      final List<ItemStack> list = getUniqueInventoryItems();
      sortItems(this.items, list);
    }
  }
