...
import static legend.game.SItem.getFirstIndexOfInventoryEntry;
import static legend.game.SItem.getInventoryEntryQuantity;
import static legend.game.SItem.getInventoryEquipmentCount;
import static legend.game.SItem.getInventoryItemCount;
import static legend.game.SItem.getRemainingInventoryItemSpace;
import static legend.game.SItem.getUniqueInventoryEquipments;
import static legend.game.SItem.getUniqueInventoryItems;
...
public class ShopScreen extends MenuScreen {
  ...
  protected void render() {
    switch(this.menuState) {
      ...
      case INIT_2 -> {
        ...
        this.selectedMenuOptionRenderable_800bdbe0 = allocateUiElement(0x7a, 0x7a, 52, this.getShopMenuYOffset(this.menuIndex_8011e0dc));
      ...
      case SELL_10 -> {
        ...
        final int count;
        final List<ItemStack> items = getUniqueInventoryItems();
        final List<Equipment> equipments = getUniqueInventoryEquipments();
        if(this.sellType != 0) {
          count = items.size();

          if(count > 0 && this.invScroll_8011e0e4 + this.invIndex_8011e0e0 < count) {
            renderString(193, 122, I18n.translate(items.get(this.invScroll_8011e0e4 + this.invIndex_8011e0e0).getDescriptionTranslationKey()), false);
          }
        } else {
          count = equipments.size();

          if(count > 0 && this.invScroll_8011e0e4 + this.invIndex_8011e0e0 < count) {
            renderString(193, 122, I18n.translate(equipments.get(this.invScroll_8011e0e4 + this.invIndex_8011e0e0).getDescriptionTranslationKey()), false);
          }
        }
  ...
  private void renderSellList(final int firstItem, final boolean isItemShop, final Renderable58 upArrow, final Renderable58 downArrow) {
    final List<?> items = isItemShop ? getUniqueInventoryItems() : getUniqueInventoryEquipments();
    if(isItemShop) {
      this.itemExtension.drawShopHeader(this, this.shop, gameState_800babc8, null, 16, 16);
      int i;
      if (!items.isEmpty()) {
        for(i = 0; firstItem + i < items.size() && i < 6; i++) {
          final ItemStack stack = (ItemStack)items.get(firstItem + i);
          final int quantity = getInventoryEntryQuantity(stack);
      ...
      if (!items.isEmpty()) {
        for(i = 0; firstItem + i < items.size() && i < 6; i++) {
          ...
          if(equipment.canBeDiscarded()) {
            final ShopSellPriceEvent event = EVENTS.postEvent(new ShopSellPriceEvent(this.shop, equipment, equipment.getPrice()));
    ...
    upArrow.setVisible(!items.isEmpty() && firstItem != 0);
  }
  ...
  protected InputPropagation mouseMove(final int x, final int y) {
    ...
    if(this.menuState == MenuState.RENDER_3) {
      ...
      final int count = this.sellType != 0 ? getUniqueInventoryItems().size() : getUniqueInventoryEquipments().size();
  ...
  private void menuSell10Select(final boolean sellStack) {
    ...
    if(list.isEmpty() || (isItem && slot >= list.size()) || (this.sellType == 0 && (slot >= list.size() || !((Equipment)list.get(slot)).canBeDiscarded()))) {
      ...
      menuStack.pushScreen(new MessageBoxQuantityScreen(I18n.translate("lod_core.ui.shop.sell", I18n.translate(entry.getNameTranslationKey())), 1, quantity, 2, result -> {
        if(Objects.requireNonNull(result.messageBoxResult) == MessageBoxResult.YES) {
          ...
          if(taken) {
            final ShopSellPriceEvent event = EVENTS.postEvent(new ShopSellPriceEvent(this.shop, entry, entry.getSellPrice()));
  ...
  private void menuSell10NavigateEnd() {
    ...
    if(this.invScroll_8011e0e4 + this.invIndex_8011e0e0 != itemCount - 1) {
      playMenuSound(1);
      this.invIndex_8011e0e0 = Math.clamp(itemCount - 1, 0, 5);
  ...
  private void setEquipCharIndex(final int index) {
    if(gameState_800babc8.equipment_1e8.isEmpty()) {
      this.equipCharIndex = 0;
    }
    else {
      this.equipCharIndex = this.FUN_8010a864(gameState_800babc8.equipment_1e8.get(index));
    }
  }
