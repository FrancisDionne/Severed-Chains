...
import static legend.game.SItem.sortItemInventory;
...
public class UseItemScreen extends MenuScreen {
  ...
  private void renderUseItemMenu(final int selectedSlot, final int slotScroll, final long a3) {
    ...
    //LAB_80102ee8
    renderMenuItems(18, 10, this.menuItems, slotScroll, 5, upArrow_800bdb94, downArrow_800bdb98, true, true);
  ...
  private void menuStage2Select() {
    ...
    if(this.itemTargetAll) {
      for(int i = 0; i < 7; i++) {
        if(Item.characterCanUseItemInMenu(i, stack.getItem())) {
  ...
  private void menuStage3Escape() {
    if(!this.itemTargetAll) {
      ...
      for(int i = 0; i < 7 && i < characterCount_8011d7c4; i++) {
        final Item item = this.menuItems.get(this.selectedSlot + this.slotScroll).item_00.getItem();
